...
class TestPost:
    ...
    def test_index_view(self, client, post_with_group):
        ...
        assert response.status_code == 200, "Главная страница работает неправильно."
        # проверка моделей
    ...
    def test_post_admin(self):
        ...
        ), "Добавьте дефолтное значение `-empty-` для пустого поля"
        assert (
...
class TestGroupView:
    ...
    def test_group_view(self, client, post_with_group):
        ...
        assert re.search(r"(p|P)ublication date:\s*", html), (
            "Отредактируйте HTML-шаблон, не найдена дата публикации "
            '`Public Date: {{ post.pub_date|date:"d E Y" }}`'
        )

        assert re.search(
            r"(a|A)uthor\:\s" + post_with_group.author.get_full_name(),
            html,
        ), (
            "Отредактируйте HTML-шаблон, не найден автор публикации "
            "`Author: {{ полное_имя_автора_поста }},`"
        )
